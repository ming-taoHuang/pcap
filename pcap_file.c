...
int main(int argc,char* argv[])
{
	...
	while(1){
		...
		if(flag==1){
			...
			snprintf(src_mac,sizeof(src_mac),"%02x:%02x:%02x:%02x:%02x:%02x",mac->ether_dhost[0],mac->ether_dhost[1],mac->ether_dhost[2],mac->ether_dhost[3],mac->ether_dhost[4],mac->ether_dhost[5]);
			snprintf(des_mac,sizeof(des_mac),"%02x:%02x:%02x:%02x:%02x:%02x",mac->ether_shost[0],mac->ether_shost[1],mac->ether_shost[2],mac->ether_shost[3],mac->ether_shost[4],mac->ether_shost[5]);
